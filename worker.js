...
// ===================== KV: CLICKS =====================
async function getClicks(env, code) {
  const raw = await env.LINKS_KV.get("clicks:" + code);
  return raw ? JSON.parse(raw) : [];
}
async function putClicks(env, code, clicks) {
...
async function isDomainBlacklisted(env, targetUrl) {
  try {
...
async function isKeywordBlacklisted(env, code, role) {
...
async function createLinkInternal(env, { url: targetUrl, owner, role, customCode, title, campaign, tags, expiryDate, customDomain, password, pixels, abUrls, abPercentages, deepLinks }) {
  ...
  } else if (expiryDate && (role === "pro" || role === "super" || role === "admin")) {
...
async function recordClick(request, env, code, link) {
  try {
...
function computeLinkAnalytics(link, clicks) {
  const now = Date.now();
...
// ===================== AUDIT LOG =====================
async function addAuditLog(env, adminUser, action, details, request) {
  try {
...
function renderNotFoundHtml(code) {
  return `<!DOCTYPE html><html><body style="background:#090d16;color:#fff;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;"><div style="text-align:center;"><h1 style="color:#f43f5e;font-size:48px;margin:0;">404</h1><p style="color:#94a3b8;">Mã liên kết <b>${code}</b> không tồn tại hoặc đã bị xóa.</p></div></body></html>`;
}
function renderDisabledHtml() {
  return `<!DOCTYPE html><html><body style="background:#090d16;color:#fff;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;"><div style="text-align:center;"><h1 style="color:#eab308;font-size:36px;margin:0;">403</h1><p style="color:#94a3b8;">Liên kết này hiện đang tạm dừng hoạt động bởi người tạo.</p></div></body></html>`;
}
function renderExpiredHtml() {
  return `<!DOCTYPE html><html><body style="background:#090d16;color:#fff;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;"><div style="text-align:center;"><h1 style="color:#f97316;font-size:36px;margin:0;">410</h1><p style="color:#94a3b8;">Liên kết này đã hết hạn hiệu lực theo chính sách gói.</p></div></body></html>`;
}
function renderWelcomeHtml() {
  return `<!DOCTYPE html><html><body style="background:#090d16;color:#fff;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;"><div style="text-align:center;max-width:500px;padding:24px;"><h1 style="color:#818cf8;margin-bottom:8px;">🚀 SHURL Cloudflare Worker Engine</h1><p style="color:#94a3b8;line-height:1.6;">Worker đang hoạt động bình thường trên mạng biên Cloudflare Edge.</p></div></body></html>`;
}
...
<nav><div class="navwrap">
  <div class="brand" id="navBrand"><img src="data:image/png;base64," alt="SHURL" style="height:24px;vertical-align:middle;margin-right:6px;"> SHORT URL</div>
  <div class="navlinks" id="navLinks" style="display:none;"></div>
</div></nav>
<div class="app-shell">
...
var i18n = {
  vi: {
    ...
    home_promo_sub:"Không thời hạn · Tên rút gọn tuỳ chỉnh · Quản lý link · Thống kê lượt click",
...
function guideCard(page){
  ...
  var ctas = { home: t("guide_home_cta"), shorturls: t("guide_shorturls_cta"), dashboard: t("guide_dashboard_cta"), bulkqr: "", analytics: "", webhooks: t("guide_webhooks_cta"), export: t("guide_export_cta"), campaigns: t("guide_campaigns_cta"), team: t("guide_team_cta"), api: t("guide_api_cta"), pricing: t("guide_pricing_cta"), account: t("guide_account_cta"), admin: t("guide_admin_cta") };
  var ctaActions = {
    home: 'document.getElementById("f_url").focus()',
    shorturls: 'document.getElementById("createForm").scrollIntoView({behavior:"smooth"})',
    dashboard: 'navigate("dashboard"); render();',
    bulkqr: '',
    analytics: '',
    webhooks: 'document.getElementById("whUrl")?document.getElementById("whUrl").focus():null',
    export: 'document.getElementById("btnExportCsv")?document.getElementById("btnExportCsv").scrollIntoView({behavior:"smooth"}):null',
    campaigns: 'navigate("campaigns"); render();',
    team: 'document.getElementById("teamName")?document.getElementById("teamName").focus():null',
    api: 'document.getElementById("btnGenToken")?document.getElementById("btnGenToken").focus():null',
    pricing: 'navigate("pricing"); render();',
    account: 'document.getElementById("acctForm")?document.getElementById("acctForm").scrollIntoView({behavior:"smooth"}):null',
    admin: 'navigate("admin"); render();'
  };
...
function renderDashboard(app){
  ...
  var canExpiry = state.user.role === "pro" || state.user.role === "super" || state.user.role === "admin";
  var isAdmin = state.user && state.user.role === "admin";
...
function upgradeBanner(pageCtx){
  if (!state.user) return "";
  if (state.user.role === "pro" || state.user.role === "super" || state.user.role === "admin") return "";
